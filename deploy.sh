...
if [ "$1" == "--setup" ]; then
  ...
  if [ -f "$DB_PATH" ]; then
    ...
    echo "   Attempting to create with explicit path..."
    DATABASE_URL="file:$DB_PATH" npx prisma db push
  fi

  echo "[7/8] Seeding admin user..."
  node scripts/seed-admin.js || echo "⚠️  Admin seeding failed (may already exist)"
...
if [ ! -f "$DB_PATH" ]; then
  ...
  DATABASE_URL="file:$DB_PATH" npx prisma db push
  node scripts/seed-admin.js || echo "⚠️  Admin seeding failed"
fi
